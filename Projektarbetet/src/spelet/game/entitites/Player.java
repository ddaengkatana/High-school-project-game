...
public class Player extends Mob{
	...
	public void tick() {
		...
		if (level.getTile(this.x/8, this.y/8).isRetrievable()&& input.space.isPressed()) {
			level.clearTile(this.x/8, (this.y/8)); 
			score++;
			}
		//if (input.space.isPressed()){
			//level.clearTile(this.x >> 3, this.y >> 3); 
			//score++;
		//}
	...
	public void render(Screen screen) {
		...
		if (Tile.FLOWER != null){
			Font.render(Integer.toString((int) Math.round(score)), screen, xOffset + 1, yOffset + 17, Colours.get(-1, -1, -1, 555), 1);
		}
	}
